...
class PeersUI {
    ...
    constructor() {
        ...
        Events.on('peer-rtc-connected', e => this._onRTCConnectable(e.detail));
    }
    ...
    _onPaste(e) {
        ...
        if (files.length > 0 && peers.length === 1) {
            Events.fire('files-selected', {
                files: files, to: $$('x-peer').id
            });
        }
    }

    _onRTCConnectable(peerId) {
        const $peer = $(peerId);
        if (!$peer) return;
        $peer.ui._onRTCConnectable();
    }
}
...
class PeerUI {
    ...
    html() {
        return `
            <label class="column center" title="点击发送文件或右键点击发送文本">
    ...
    _initDom() {
        ...
        this.$icon = el.querySelector('x-icon');
    }
    ...
    _onRTCConnectable() {
        this.$icon.style.background = 'var(--primary-color)';
...
class SendTextDialog extends Dialog {
    ...
    _send(e) {
        ...
        Events.fire('send-text', {
            to: this._recipient, text: this.$text.innerText
        });
    }
}
...
class Notifications {
    ...
    _requestPermission() {
        Notification.requestPermission(permission => {
            if (permission !== 'granted') {
                if (language === 'zh') {
                    Events.fire('notify-user', Notifications.PERMISSION_ERROR_ZH || 'Error');
                } else {
                    Events.fire('notify-user', Notifications.PERMISSION_ERROR_EN || 'Error');
                }
                return;
            }
    ...
    _notify(message, body) {
        const config = {
            body: body, icon: '/images/logo_transparent_128x128.png',
        }
        let notification;
        try {
...
Notifications.PERMISSION_ERROR_ZH = `通知权限已被阻止，因为用户多次关闭了权限提示。可以在“页面信息”中重置权限，点击URL旁边的锁形图标访问。`;
Notifications.PERMISSION_ERROR_EN = `Notifications permission has been blocked as the user has dismissed the permission prompt several times. This can be reset in Page Info which can be accessed by clicking the lock icon next to the URL.`;
